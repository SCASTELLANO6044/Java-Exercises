...
public class QuickSortTest {
    ...
    public static void main(String[] args) {
        runTest("Null input returns null", () -> assertSort(null, (int[]) null));
        ...
        runTest("Repeated pivot values surrounded by smaller and larger values", () -> {
            assertSort(new int[]{5, 2, 5, 8, 5, 1, 9, 5}, 1, 2, 5, 5, 5, 5, 8, 9);
            assertSort(new int[]{2, 1, 2, 1, 2, 1}, 1, 1, 1, 2, 2, 2);
        });
        runTest("Larger sorted, reversed, and all-equal arrays", () -> {
            int[] ascending = new int[256];
            int[] descending = new int[256];
            int[] equal = new int[256];
            for (int i = 0; i < ascending.length; i++) {
                ascending[i] = i;
                descending[i] = ascending.length - 1 - i;
                equal[i] = 7;
            }
            int[] expected = ascending.clone();
            assertSort(ascending, expected);
            assertSort(descending, expected);
            assertSort(equal, equal.clone());
        });
        ...
        if (failed > 0) {
            throw new AssertionError("Some tests failed. Check your quicksort implementation.");
        }
    }
    ...
    private static void assertSort(int[] numbers, int... expected) {
        ...
        if (result != numbers) {
            throw new AssertionError("Expected the original array reference, or null for null input.");
        }
